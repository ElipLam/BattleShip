...
*/
	...
	public function get_board_html($type = 'first', $direct = false)
	{
		...
		$html = '<table class="board '.$type.' '.$color.'">';
		$top = '<tr class="row top"><td class="corner"></td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td><td>6</td><td>7</td><td>8</td><td>9</td><td>10</td><td class="corner"></td></tr>';
		...
		for ($i = 0; $i < strlen($board); ++$i) {
			...
			$side = '<td class="side">'.$letters[(int) floor($i / 10)].'</td>';

			// add the border
			if (0 == ($i % 10)) {
				$html .= '<tr class="row">'.$side;
			}
			...
			// put in the div
			$html .= "<td{$tgt_id}{$class} onclick='tdclick(this);'>{$img}</td>";
		...
		$html .= str_replace('top', 'bottom', $top);
		$html .= "\n</table>";

		// if there is no board
		if (100 != strlen($board)) {
			// let the opponent know about it
			$html = '<table class="noboard">This player has not set up their board yet.</table>';
		}
		call($html);
