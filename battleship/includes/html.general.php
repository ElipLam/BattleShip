...
function get_header($meta = null) {
	...
	if ($show_nav_links && class_exists('Settings') && Settings::test( )) {
		$nav_links = '<a class="logout100" href="login.php'.$GLOBALS['_?_DEBUG_QUERY'].'">Logout</a>';
	}
...
<head>
	...
	{$admin_css}
	<link rel="icon" type="image/png" href="images/icons/18ac13.png"/>
</head>

<body id="{$file_name}_page">
	{$admin_div}
		<div class="head">
		<div class="button100">{$nav_links}</div>
		
EOF;
	//<h1><a href="index.php">{$GAME_NAME}</a></h1>
	if ($show_menu) {
		$html .= '
	<div class="head1" id="nav">';

		if ($menu_data) {
			$html .= '
		<table id="menu">
			<tr>
				<th'.get_active('index').'><a class="nav100" href="index.php'.$GLOBALS['_?_DEBUG_QUERY'].'" '.(($allow_blink && $menu_data['my_turn']) ? ' class="blink"' : '').'>Lobby</a></th> 
				<th'.get_active('invite').'><a class="nav100" href="invite.php'.$GLOBALS['_?_DEBUG_QUERY'].'" title="(Received | Sent)"'.(($allow_blink && $menu_data['in_vites']) ? ' class="blink"' : '').'>Invitations <span class="sep">(</span> '.$menu_data['in_vites'].' <span class="sep">|</span> '.$menu_data['out_vites'].' <span class="sep">)</span></a></th>
				<th><a class="nav100" href="gamepad.php" class="blink">Gamepads</a></th> 
				';
	...
	// <span class="sep">(</span> '.$menu_data['my_turn'].' <span class="sep">|</span> '.$menu_data['my_games'].' <span class="sep">|</span> '.$menu_data['games'].' <span class="sep">)</span>
	}
	$html .= '</div><!-- #head -->';
	return $html;
}
